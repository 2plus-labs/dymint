...
func NewExecutor(ctx context.Context, logger log.Logger, client *tplus.TplusClient, sender string, maxBatch int, msgQueue *queue.Queue) *Executor {
	e := &Executor{
		ctx:                   ctx,
		logger:                logger,
		client:                client,
		ctlRoundRetryAttempts: 5,
		ctlRoundRetryDelay:    1 * time.Second,
...
func (e *Executor) Serve(ctx context.Context) error {
	//for {
	//	//e.logger.Info("executor: ", "sender", e.sender, "waiting for msgs")
	//	items, found := e.msgs.PopByTime(time.Now().Unix(), e.maxBatch)
	...
	//				return err
	//			}
	//			e.client.Factory.WithSequence(accSeq).WithAccountNumber(accNum)
	//			return err
	//		}
	//		return nil
